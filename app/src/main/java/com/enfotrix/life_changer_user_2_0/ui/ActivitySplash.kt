        ...
        sharedPrefManager = SharedPrefManager(mContext)
         ...
         Timer().schedule(1500){
             ...
             if(sharedPrefManager.isLoggedIn()==true){
                 if(sharedPrefManager.getStatus().equals(constants.INVESTOR_STATUS_ACTIVE)||sharedPrefManager.getUser().status.equals(constants.INVESTOR_STATUS_PENDING))
                 {

                     startActivity(Intent(mContext,MainActivity::class.java)) // shared pref -> token
                     finish()


                 }
                 else if(sharedPrefManager.getStatus().equals(constants.INVESTOR_STATUS_INCOMPLETE) ){
                     startActivity(Intent(mContext,ActivityUserDetails::class.java)) // shared pref -> token
                     finish()
                 }
